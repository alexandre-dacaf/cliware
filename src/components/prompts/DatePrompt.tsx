...
const DatePrompt: React.FC<DatePromptProps> = ({ message, onSubmit, isActive, onEscape }) => {
    ...
    const { date, yearRef, monthRef, dayRef, yearHandler, monthHandler, dayHandler, focusYear, clear } = useDatePrompt(year, month, day);
    const { submit } = usePromptSubmitter(message, onSubmit);

    const handleEnter = () => {
        submit({ data: date, clear });
    };

    const handleKeyDown = (handlers: DateKeyDownHandler) => (event: React.KeyboardEvent<HTMLSpanElement>) => {
        const key = event.key;
